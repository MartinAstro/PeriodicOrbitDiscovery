...
def main():
    ...
    # Shooting solvers
    bounds = ([0.7, 0.1, -np.pi, -2*np.pi, -2*np.pi, -2*np.pi, 0.9],
              [1.0, 0.5, np.pi, 2*np.pi, 2*np.pi, 2*np.pi, 1.1])
    ...
    solver = ShootingLsSolver(lpe, decision_variable_mask, constraint_variable_mask) # Finds a local optimum, step size gets too small
