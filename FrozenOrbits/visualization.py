...
def __plot_TraditionalOE(t, OE, **kwargs):
    ...
    element_labels = ["$a$", "$e$", "$i$", "$\omega$", "$\Omega$", "$M$"]
...
def plot_OE_1d(t, OE, element_set, **kwargs):
    ...
    if element_set.lower() == "traditional":
        ...
        raise NotImplementedError(
            element_set + " is not a currently supported orbital element set!"
        )
...
def plot_cartesian_state_3d(X, obj_file=None, **kwargs):
    ...
    if obj_file is not None:
        ...
        tri = Poly3DCollection(
            mesh.triangles * 1000, cmap=cmap, facecolors=cmap(128), alpha=0.5
        )
