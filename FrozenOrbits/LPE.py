...
class LPE_Traditional_ND_2():
    "LPE with traditional OE, but normalized by length and time"
    def __init__(self, model, mu, l_star, t_star):
        self.model = model
        self.l_star = tf.constant(l_star, dtype=tf.float64, name='ref_length')
    ...
    def dOE_trad_dt(self, OE, mu): 
        with tf.GradientTape(persistent=True) as tape:
            ...
            u_pred = (self.t_star/self.l_star)**2*u_pred_tilde
        dUdOE = tape.gradient(u_pred, OE)
...
class LPE_Traditional_ND_3():
    "LPE with traditional OE, but normalized by length and time"
    def __init__(self, model, mu, l_star, t_star, m_star):
        self.model = model
        self.l_star = tf.constant(l_star, dtype=tf.float64, name='ref_length')
        self.t_star = tf.constant(t_star, dtype=tf.float64, name='ref_time')
