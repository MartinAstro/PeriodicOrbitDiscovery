...
def main():
    ...
    lpe = LPE_Milankovitch(
        ...
        planet.mu,
        l_star=np.sqrt(
            T_dim * np.linalg.norm(OE_0_dim[0, 0:3]) / 1.0
        ),  # H = t/l**2 * H_tilde
        t_star=T_dim,
        m_star=1.0,
    )
    ...
    # Configuration 2
    OE_0_dim = np.array(
        [[-1.51e04, -9.02e04, 9.56e04, 5.41e-02, -8.54e-02, -8.92e-02, 1.21e00]]
    )
    X_0_dim = oe2cart_tf(OE_0_dim, planet.mu, element_set="milankovitch").numpy()
    ...
    lpe = LPE_Milankovitch(
        ...
        planet.mu,
        l_star=np.sqrt(
            T_dim * np.linalg.norm(OE_0_dim[0, 0:3]) / 10.0
        ),  # H = t/l**2 * H_tilde
        t_star=T_dim,
        m_star=1.0,
    )
    ...
    OE_lpe_dim_diff, OE_lpe_diff = print_OE_differences(
        OE_sol.y.T, lpe, "OE Integration", constraint_angle_wrap
    )
    OE_cart_dim_diff, OE_cart_diff = print_OE_differences(
        init_sol.y.T, lpe, "Cartesian Integration", constraint_angle_wrap
    )

    cart_IC_a = oe2cart_tf(
        init_sol.y[:, 0].reshape((1, -1)), planet.mu, element_set="milankovitch"
    ).numpy()[0]
    cart_IC_b = oe2cart_tf(
        OE_sol.y[:, 0].reshape((1, -1)), planet.mu, element_set="milankovitch"
    ).numpy()[0]
    assert np.allclose(
        cart_IC_a, cart_IC_b
