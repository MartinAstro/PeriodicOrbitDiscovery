...
def plot_integrated_orbits(coarse_df, fine_df, base_figure_name):
    ...
    for k in range(3):
        ...
        original_orbit_sol = propagate_orbit(
            T_0, X_0, model, tol=1e-7, t_eval=t_eval_original
        )
        plot_cartesian_state_3d(
            original_orbit_sol.y.T,
            planet.obj_8k,
            new_fig=True,
            solid_color="black",
            line_opacity=0.5,
        ...
        solution_orbit_sol = propagate_orbit(
            T_0_sol, X_0_sol, model, tol=1e-7, t_eval=t_eval_solution
        )
        plot_cartesian_state_3d(
            solution_orbit_sol.y.T,
            None,
            new_fig=False,
            solid_color=solid_color_list[k],
            fig_size=vis.fig_size,
        )
        ...
        t_eval_solution = np.linspace(
            0, T_0_sol * period_multiplier, 100 * period_multiplier
        )
        solution_orbit_sol = propagate_orbit(
